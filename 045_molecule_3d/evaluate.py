...
def calc_molformer(df):
    mf = pd.read_csv("molformer/molformer_embed.csv")
    df = pd.merge(df, mf, left_on="smiles", right_on="SMILES", how="left")
    molformer_cols = [c for c in df.columns if c.startswith("molformer_")]
    df["fp"] = df[molformer_cols].values.tolist()
    return df
...
def calc_descriptors(df_train, df_test, desc_name):
    ...
    elif desc_name == "molformer":
        df_train = calc_molformer(df_train)
        df_test = calc_molformer(df_test)
    else:
...
if split == "ood_dG":
    ...
    df_train, df_test = split_random(df, test_size=0.2)  # random split
else:
    raise ValueError("split must be 'ood_dG' or 'random'")
...
df_results.to_csv(f"results_{split}.csv", index=False)
